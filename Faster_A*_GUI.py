...
def print_until_quit(SCREEN):
    ...
    button = pygame.draw.rect(SCREEN,GREEN,(temp1,temp2,105,50))
    while(1):
        ...
        for i in pygame.event.get():
            if i.type == pygame.QUIT:
                ...
                if button.collidepoint(pos):
                    button = pygame.draw.rect(SCREEN,DARK_GREEN,(temp1,temp2,105,50))
                else:
                    button = pygame.draw.rect(SCREEN,GREEN,(temp1,temp2,105,50))
...
def evalfn(row,col,parent_row,parent_col):                   # f(n) for each state (row,col)
    print "Parents are" ,parent_row,parent_col,"called by",row,col
    print g_val[parent_row][parent_col]
...
def solver(SCREEN):
    ...
    while(1):
        ...
        board[ele[0]][ele[1]] = 'V'
        # Updating the color of the visited block to White
        rect[ele[0]][ele[1]] = pygame.draw.rect(SCREEN,WHITE,(((MARGIN*(ele[1]+1)) + (ele[1]*BLOCK_WIDTH)),((MARGIN*(ele[0]+1)) + (ele[0]*BLOCK_WIDTH)),BLOCK_WIDTH,BLOCK_HEIGHT))
        ...
        for child in children:
            ...
            if board[row1][col1]=='F':      # F,G ( V,S,W cannot occur here )
                ...
                rect[row1][col1] = pygame.draw.rect(SCREEN,BLUE,(((MARGIN*(col1+1)) + (col1*BLOCK_WIDTH)),((MARGIN*(row1+1)) + (row1*BLOCK_WIDTH)),BLOCK_WIDTH,BLOCK_HEIGHT))
                board[row1][col1] = 'F'
...
def controller():
    ...
    while(1):
        ...
        g_val[r_s][c_s] = 0
        print "r_s and c_s are",r_s,c_s
        print "r_g and c_g are",r_g,c_g
        solver(screen)
